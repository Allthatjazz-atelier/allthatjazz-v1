...
/**
 * Elige la misma URL que `<img srcset>` para un ancho en px de dispositivo
 * (CSS px × DPR). Lo usan Grid (vía el navegador) y Galaxy en detalle.
 */
export function pickImageUrlForDisplayWidth(imageSet, devicePx) {
  if (!imageSet?.src) return null;
  if (!imageSet.srcSet) return imageSet.src;
  const parts = imageSet.srcSet
    .split(", ")
    .map((chunk) => {
      const sp = chunk.lastIndexOf(" ");
      if (sp <= 0) return null;
      const w = parseInt(chunk.slice(sp + 1), 10);
      return w > 0 ? { url: chunk.slice(0, sp), w } : null;
    })
    .filter(Boolean)
    .sort((a, b) => a.w - b.w);
  if (!parts.length) return imageSet.src;
  const need = Math.max(1, Math.round(devicePx));
  return parts.find((p) => p.w >= need)?.url ?? parts[parts.length - 1].url;
}
