...
// ── Componente ────────────────────────────────────────────────────────────────
export default function HeaderFooter12({ children }) {
  ...
  useEffect(() => {
    ...
    const el   = h1Ref.current;
    if (!turb || !disp || !blur || !el) return;

    gsap.killTweensOf(["#atj-turb", "#atj-disp", "#atj-blur"]);
    ...
    if (hovered) {
      // Hover ON — distorsión suave que engrosa bordes como mancha de tinta
      gsap.to(state, {
        scale: 18,   // desplazamiento fino, no exagerado
        blur:  1.2,  // blur muy leve — solo suaviza los bordes distorsionados
        freq:  0.018,
        duration: 0.7,
        ease: "power2.out",
        onUpdate() {
        ...
        onComplete() {
          // Respiración lenta y orgánica — muy sutil
          gsap.to(state, {
            scale: 14,
            freq:  0.012,
            duration: 1.8,
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 w-full flex flex-col justify-center items-center pb-2 leading-[2.75rem] z-[9999] HeaderFooter select-none pointer-events-auto">
        ...
        <svg width="0" height="0" style={{ position: "absolute", overflow: "hidden" }}>
          <defs>
            <filter id="atj-filter" x="-20%" y="-20%" width="140%" height="140%">
              {/*
                feTurbulence genera ruido fractal orgánico de baja frecuencia.
                Frecuencia baja → ondas largas y suaves, no ruido agresivo.
                seed fijo → el patrón es siempre el mismo al arrancar.
              */}
              <feTurbulence
                id="atj-turb"
                type="fractalNoise"
              ...
              />
              {/*
                feDisplacementMap desplaza los píxeles del texto según el ruido.
                scale=0 en reposo → invisible. scale~18 en hover → bordes ondulados.
                Con valores bajos el texto parece "crecer" irregularmente
                en lugar de distorsionarse agresivamente.
              */}
              <feDisplacementMap
                id="atj-disp"
                in="SourceGraphic"
              ...
              />
              {/*
                feGaussianBlur muy leve sobre el desplazado — suaviza
                los bordes irregulares para que parezca tinta o mancha
                en lugar de pixelación.
              */}
              <feGaussianBlur
                id="atj-blur"
                in="displaced"
