      ...
      <div className="relative w-full h-full" style={{ perspective: '1000px' }}>
        {images.map((img, index) => (
          <div
            ...
            className={`absolute ${img.position} ${img.z} transition-all duration-100`}
            style={{ 
              width: img.size,
              filter: `
                ...
                hue-rotate(${effects.glitch > 0.5 ? effects.glitch * 180 : 0}deg)
                invert(${effects.invert})
              `,
              transform: `
                rotate(${effects.rotate * 15}deg)
                translateX(${effects.shake * (Math.random() - 0.5) * 20}px)
                translateY(${effects.shake * (Math.random() - 0.5) * 20}px)
                ${effects.wave > 0 ? `translateY(${Math.sin(Date.now() / 500 + index) * effects.wave * 30}px)` : ''}
                scale(${1 + effects.scale * 0.5})
                skew(${effects.skew * 20}deg, ${effects.skew * 10}deg)
                scaleX(${effects.flip > 0.5 ? -1 : 1})
                rotateY(${effects.perspective * 60}deg)
                rotateX(${effects.perspective * 30}deg)
              `,
