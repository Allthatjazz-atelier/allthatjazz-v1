...
const Slider3 = () => {
  ...
  useEffect(() => {
    ...
    sequence.forEach((step) => {
      if (step.show) tl.to(step.show, { opacity: 1, duration: 0 });
      if (step.hide) tl.to(step.hide, { opacity: 0, duration: 0 });
      if (step.wait) tl.to({}, { duration: step.wait });
    });
  ...
  return (
    <div className="w-screen h-screen overflow-hidden bg-white flex flex-col justify-start items-center pt-12">
      {/* Contenedor del slider con altura controlada */}
      <div
        ref={sliderRef}
        className="relative w-11/12 h-[70vh]" // 👈 clave: controlas el bloque
      >
