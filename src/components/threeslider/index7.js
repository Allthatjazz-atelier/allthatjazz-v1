...
const HeroCarousel6 = () => {
  ...
  useEffect(() => {
    ...
    // --- FONDO CON RUIDO ESTÁTICO TIPO TV 90s ---
    const staticShader = {
      vertexShader: `
        varying vec2 vUv;
        void main() {
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Efecto de escanlines horizontales
          float scanline = sin(vUv.y * 800.0) * 0.04;
          
          // Ruido estático con un poco de color vintage
          vec3 color = vec3(noise * 0.4 + 0.3);
          color += scanline;
          
          // Tinte ligeramente azulado/verdoso tipo TV vieja
          color.g += 0.05;
          color.b += 0.08;
    ...
    const staticGeometry = new THREE.PlaneGeometry(50, 50);
    ...
    const staticBackground = new THREE.Mesh(staticGeometry, staticMaterial);
    staticBackground.position.z = -5;
    ...
    const createSlide = (index) => {
      ...
      // Shader custom para integrar el ruido con las imágenes
      const slideMaterial = new THREE.ShaderMaterial({
        uniforms: {
          map: { value: null },
          time: { value: 0 },
        ...
        fragmentShader: `
          ...
          void main() {
            ...
            vec2 noiseCoord = vUv * 200.0 + time;
            float noise = random(noiseCoord) * noiseIntensity;
            color.rgb += noise;
            ...
            // Tinte ligeramente azulado en las sombras
            color.b += 0.02 * (1.0 - color.r);
      ...
      const mesh = new THREE.Mesh(geometry, slideMaterial);
      ...
      new THREE.TextureLoader().load(
        ...
        (texture) => {
          ...
          slideMaterial.uniforms.map.value = texture;
        ...
        if (selectedSlide === clickedSlide) {
          ...
          slides.forEach(slide => {
            if (slide === selectedSlide) {
              slide.userData.targetScale = 1.15;
            } else {
              slide.userData.targetScale = 0.15;
            }
          });
        }
      }
    ...
    const animate = (time) => {
      ...
      // Actualizar ruido estático del fondo
      staticMaterial.uniforms.time.value = time * 0.05;
      ...
      slides.forEach((slide, i) => {
        ...
        // Actualizar tiempo del shader de la slide
        if (slide.material.uniforms && slide.material.uniforms.time) {
          slide.material.uniforms.time.value = time * 0.05;
        }
