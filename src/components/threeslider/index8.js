...
const HeroCarousel7 = () => {
  ...
  useEffect(() => {
    ...
    // --- FONDO CON RUIDO TV 90s (MÁS SUAVE, ANALÓGICO) ---
    const staticShader = {
      vertexShader: `
        varying vec2 vUv;
        void main() {
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Drift vertical muy leve (CRT)
          uv.y += sin(time * 0.15 + uv.x * 2.0) * 0.002;

          // Ruido analógico suave
          float n = noise(uv * 80.0 + time * 0.05);
          n = smoothstep(0.35, 0.65, n);

          // Scanlines muy sutiles
          float scanline = sin(uv.y * 600.0) * 0.015;

          vec3 color = vec3(0.9 + n * 0.1);
          color += scanline;
