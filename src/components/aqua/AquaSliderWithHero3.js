...
// ─── Main component ────────────────────────────────────────────────────────────
const AquaSliderWithHero3 = () => {
  const canvasRef = useRef(null);
  const [isMobile, setIsMobile] = useState(false);

  // ── Resolve image paths ───────────────────────────────────────────────
  // Static fallback: uses /story/ paths directly.
  // To plug in useOptimizedMedia, replace this with:
  //   const { getImage, isLoaded } = useOptimizedMedia();
  //   if (!isLoaded) return null inside the Three.js useEffect check.
  const resolvedGroups = IMAGE_NAMES.map((group) =>
    group.map((name) => `/story/${name}`)
  );
  ...
  useEffect(() => {
    ...
    // Concentric sine waves travel outward from each slide's center,
    // driven by scroll velocity. Envelope fades to 0 at edges so slides
    // are always flat when at rest.
    //
    //   z = sin(d × freq − t × waveSpeed) × amplitude × envelope
    //
    // envelope = max(0, 1 − d/radius)^falloff
    // amplitude = distortionFactor × maxDistortion  (0 when not scrolling)

    const WAVE = {
      frequency:  isMobile ? 4.5 : 5.5,   // wave crests per world unit
      speed:      isMobile ? 3.5 : 4.5,   // outward travel speed (units/s)
      falloff:    isMobile ? 1.6 : 1.8,   // envelope sharpness
      radius:     isMobile ? slideHeight * 1.8 : 3.2, // envelope radius
      secondary:  0,                    // 2nd harmonic ratio (organic texture)
      secFreqMul: 2.1,                     // 2nd harmonic frequency multiplier
    };

    const updateCurve = (mesh, worldPosition, distortionFactor, elapsedSec) => {
      const positionAttr = mesh.geometry.attributes.position;
      const original     = mesh.userData.originalVertices;
      const amplitude    = settings.maxDistortion * distortionFactor;

      for (let i = 0; i < positionAttr.count; i++) {
        const lx = original[i * 3];
        const ly = original[i * 3 + 1];

        // World-space coords relative to slide center
        const wx = isVertical ? lx               : worldPosition + lx;
        const wy = isVertical ? worldPosition + ly : ly;
        const d  = Math.sqrt(wx * wx + wy * wy);

        // Smooth envelope: full at center, zero at radius
        const env = Math.pow(Math.max(0, 1 - d / WAVE.radius), WAVE.falloff);

        // Primary wave + 2nd harmonic for organic feel
        const phase   = d * WAVE.frequency - elapsedSec * WAVE.speed;
        const primary = Math.sin(phase);
        const second  = Math.sin(phase * WAVE.secFreqMul) * WAVE.secondary;

        positionAttr.setZ(i, (primary + second) * amplitude * env);
      }
    ...
    const animate = (time) => {
      ...
      slides.forEach((slide, i) => {
        ...
        updateCurve(slide, worldPos, currentDistortionFactor, elapsedTime);
      });
...
export default AquaSliderWithHero3;