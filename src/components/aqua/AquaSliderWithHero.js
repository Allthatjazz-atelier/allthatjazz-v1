...
const aquaFragmentShader = `
  ...
  void main() {
    ...
    LensDistortion distortion = getLensDistortion(
      p, uv2, sphereCenter, bubbleRadius, 0.25
    );
...
// ─── Image groups: 5 slides × 3 images ────────────────────────────────────────
const SLIDE_GROUPS = [
...
// ─── AquaSlider factory ────────────────────────────────────────────────────────
// offW / offH deben coincidir con el aspect ratio del slide del carrusel:
//   desktop → 2.0 / 2.5 = 0.8   → 512 × 640
//   mobile  → 4.0 / 4.5 ≈ 0.889 → 512 × 576
const createAquaSlider = (imageGroup, offW, offH) => {
  ...
  const textures = new Array(imageGroup.length).fill(null);
  let loadedCount = 0;
  ...
  imageGroup.forEach((src, i) => {
    loader.load(src, (tex) => {
      ...
      if (loadedCount === imageGroup.length) {
...
const AquaSliderWithHero = () => {
  ...
  useEffect(() => {
    ...
    // Offscreen canvas dimensions match the slide aspect ratio exactly
    // desktop: 2/2.5 = 0.8  → 512×640
    // mobile:  4/4.5 ≈ 0.889 → 512×576
    const offW = 512;
    const offH = isMobile ? 576 : 640;

    const slideCount = SLIDE_GROUPS.length; // 5
    ...
    // ── Create one AquaSlider per carousel slot ────────────────────────────
    const aquaSliders    = SLIDE_GROUPS.map((group) => createAquaSlider(group, offW, offH));

    // ── CanvasTexture: bridges the offscreen WebGL canvas → carousel mesh ──
    ...
    // ── Raycaster for click / tap → advance AquaSlider ────────────────────
    ...
    // ── Distortion / curve (original logic — untouched) ────────────────────
    ...
    // ── Interaction: advance the clicked / tapped AquaSlider ──────────────
    const advanceSlideUnderPointer = (clientX, clientY) => {
      mouse.x = (clientX / window.innerWidth)  *  2 - 1;
      mouse.y = (clientY / window.innerHeight) * -2 + 1;
      raycaster.setFromCamera(mouse, camera);
      const hits = raycaster.intersectObjects(slides);
      if (hits.length > 0) {
        const idx = slides.indexOf(hits[0].object);
        if (idx >= 0) aquaSliders[idx].advance();
      }
    };

    const handleClick = (e) => advanceSlideUnderPointer(e.clientX, e.clientY);

    const handleKeyDown = (e) => {
      const back    = isVertical ? e.key === "ArrowUp"   : e.key === "ArrowLeft";
      const forward = isVertical ? e.key === "ArrowDown" : e.key === "ArrowRight";
      if (back)    { targetPosition += slideUnit; targetDistortionFactor = Math.min(1.0, targetDistortionFactor + 0.3); }
      if (forward) { targetPosition -= slideUnit; targetDistortionFactor = Math.min(1.0, targetDistortionFactor + 0.3); }
    };
    ...
    const handleTouchEnd = (e) => {
      ...
      // Tap (not a swipe) → advance the AquaSlider under the finger
      if (dx < 10 && dy < 10) {
        advanceSlideUnderPointer(e.changedTouches[0].clientX, e.changedTouches[0].clientY);
        return;
      }
    ...
    const animate = (time) => {
      ...
      if (currentVelocity > 0.05) {
        targetDistortionFactor = Math.max(targetDistortionFactor, Math.min(1.0, currentVelocity * 0.1));
      }
      ...
      // Render each AquaSlider into its offscreen canvas, then flag texture for upload
      aquaSliders.forEach((slider, i) => {
        slider.render();
        canvasTextures[i].needsUpdate = true;
      });

      // Update carousel slide positions + distortion
...
export default AquaSliderWithHero;